...
> **📝 TODO**: Publish to PyPI for `pip install testability-analyzer` availability
...
python3 -m testability_analyzer.cli myfile.py

# Analyze a directory
testability-analyzer src/

# If the console entrypoint isn't available in your environment:
python3 -m testability_analyzer.cli src/
...
This repository includes a curated set of examples under `tests/examples/` that
exercise the analyzer across score bands.

```bash
# Analyze all examples
python3 -m testability_analyzer.cli tests/examples/ --verbose

# Validate that band-targeted examples fall within their expected ranges
